...
/// Reading a browser window through macOS Accessibility.
///
/// # Why this rather than pixels
///
...
/// them. So a caller can be told `button named "Sign in"` instead of matching
/// pixels, and what it reads survives a site restyling its markup as long as
/// the accessible role and name hold.
///
/// # Reading only
///
/// This drives nothing. There was once an acting half here — press, set value,
/// scroll, go back — and a Chrome backend over the DevTools protocol beside it,
/// and both are gone. The browser on screen is the user's own: it holds their
/// live sessions, and a synthesized click lands in the window they are looking
/// at, on a link nobody chose deliberately. Reading their page costs them
/// nothing and cannot misfire, so reading is all that is left.
...
/// One thing on the page the caller can reason about.
public struct BrowserElement: Sendable {
    ...
    /// Where the fact came from. Always "ax" now — kept because a reply that
    /// says where it got its facts stays readable if another reader is ever
    /// added beside this one.
    public var source: String

    public init(
        id: String, role: String, name: String, value: String?, enabled: Bool, focused: Bool,
        frame: CGRect?, source: String = "ax"
    ) {
        self.id = id
        self.role = role
        self.name = name
...
public struct BrowserSnapshot: Sendable {
    ...
    public init(
        app: String, title: String, url: String?, elements: [BrowserElement],
        truncated: Bool
    ) {
        self.app = app
        self.title = title
...
/// Reads the frontmost window of a supported browser.
///
/// Element ids are only meaningful within the observation that produced them:
/// a page re-renders constantly, so "e17" names the seventeenth thing *that
/// read* found and nothing more durable. There is no epoch guarding them any
/// more, because nothing here takes an id back — an id is a label for talking
/// about the listing, not a handle for acting on the page.
@MainActor
public final class BrowserAXSession {
    ...
    public func observe(
        ...
        func walk(_ node: AXUIElement) {
            ...
            if let role = Self.roleNames[rawRole] {
                ...
                // A nameless, valueless node says nothing and is not worth a
                // line in the listing.
                let worthReporting =
                ...
                if worthReporting {
                    elements.append(
                        BrowserElement(
                            id: "e\(elements.count + 1)", role: role, name: name,
        ...
        return BrowserSnapshot(
            ...
            url: webAreaURL, elements: elements, truncated: truncated)
    }
    ...
    // `nonisolated` because these only call AXUIElementCopyAttributeValue,
    // which is safe off the main thread — the main-actor requirement on this
    // class comes from AppKit, not from Accessibility.
